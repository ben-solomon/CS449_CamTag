...
public class DBhandler extends SQLiteOpenHelper{
    ...
    // links img_id with selected tag id
    ...
    // links img_id with selected tag ids
    ...
    public ArrayList<String> getImageTags(String fname){
        ArrayList<String> tl = new ArrayList<String>();
        SQLiteDatabase db = this.getReadableDatabase();
        String select = "select distinct Tags.TAG_NAME from Tags join Links on Links.TAG_ID = Tags.TAG_ID join Images on Links.IMAGE_ID = (SELECT IMAGE_ID FROM Images WHERE IMAGE_FILENAME='"+ fname+"')";
        Cursor c = db.rawQuery(select, null);
        if(c.moveToFirst())
        {
            do{
...
public List<String> getImageList(){
    ...
    public void deleteTag(String s){
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("DELETE FROM "+ LINK_TABLE +" WHERE TAG_ID=(SELECT TAG_ID FROM Tags WHERE TAG_NAME='"+s+"');DELETE FROM "+ TAG_TABLE +" WHERE TAG_NAME = '" + s.trim() +"';");

    }
    public void deleteImage(String fname){
        SQLiteDatabase db = this.getWritableDatabase();
        db.execSQL("DELETE FROM "+ IMAGE_TABLE +" WHERE IMAGE_FILENAME ='"+fname+"';");
    }
    public void deleteImageLinks(String fname){
        SQLiteDatabase db = this.getWritableDatabase();
        String ret;
        Cursor c = db.rawQuery("SELECT IMAGE_ID FROM Images WHERE IMAGE_FILENAME = '" + fname.trim() +"'",null);
        if(c.moveToFirst()){
           ret = c.getString(0);
            db.execSQL("DELETE FROM "+ LINK_TABLE +" WHERE IMAGE_ID ='"+ ret +"';");
        }
